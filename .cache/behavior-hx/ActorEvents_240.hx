...
class ActorEvents_240 extends ActorScript
{
	public var _time:Bool;
		...
		if(wrapper.enabled && 5 == mouseState)
		{
			if((_time == true))
			{
				if((actor.getAnimation() == "on"))
				{
					...
					Engine.engine.setGameAttribute("disablezombie", false);
				}
				else
				{
					actor.setAnimation("on");
					Engine.engine.setGameAttribute("disablezombie", true);
				}
			}
		}
	}
	...
	public function _event_Drawing(g:G, x:Float, y:Float):Void
	{
		if(wrapper.enabled && true)
		{
			...
			{
				g.drawString("" + Engine.engine.getGameAttribute("disablezombie"), 20, 10);
			}
		}
	}
