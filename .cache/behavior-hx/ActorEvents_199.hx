...
class ActorEvents_199 extends ActorScript
{
	...
	public function _event_Updating(elapsedTime:Float):Void
	{
		if(wrapper.enabled && true)
		{
			...
			{
				createRecycledActorOnLayer(getActorType(195), (Engine.engine.getGameAttribute("x") : Float), (Engine.engine.getGameAttribute("y") : Float), engine.getLayerById(Std.int(3)));
				recycleActor(actor);
				return;
			}
