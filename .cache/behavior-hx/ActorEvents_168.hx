...
class ActorEvents_168 extends ActorScript
{
	...
	public function _event_Updating(elapsedTime:Float):Void
	{
		if(wrapper.enabled && true)
		{
			...
			if(((Engine.engine.getGameAttribute("bigisactive") : Bool) == false))
			{
				recycleActor(actor);
			}
			else
			{
				if(!((Engine.engine.getGameAttribute("skin") : Float) == 1))
				{
					recycleActor(actor);
				}
			}
			if((((Engine.engine.getGameAttribute("bigtime") : Float) > 0) && (((Engine.engine.getGameAttribute("bigtime") : Float) < 0.1) && ((Engine.engine.getGameAttribute("bigisactive") : Bool) == true))))
			{
				Engine.engine.setGameAttribute("x", actor.getX());
				Engine.engine.setGameAttribute("y", actor.getY());
			}
			if((((Engine.engine.getGameAttribute("bigisdisabled01S") : Bool) == true) && ((Engine.engine.getGameAttribute("bigisbuy") : Bool) == true)))
			{
				createRecycledActorOnLayer(getActorType(166), (Engine.engine.getGameAttribute("x") : Float), (Engine.engine.getGameAttribute("y") : Float), engine.getLayerById(Std.int(3)));
				recycleActor(actor);
				return;
			}
	...
	public function _event_Drawing(g:G, x:Float, y:Float):Void
	{
		if(wrapper.enabled && true)
		{
			...
			if(((Engine.engine.getGameAttribute("devmode") : Bool) == true))
			{
				...
				g.drawString("" + Engine.engine.getGameAttribute("x"), 100, 100);
				g.drawString("" + Engine.engine.getGameAttribute("y"), 100, 150);
			}
		}
	}
