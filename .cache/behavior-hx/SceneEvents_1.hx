...
class SceneEvents_1 extends SceneScript
{
	...
	public function _event_AfterNsecs(timeTask:TimedTask):Void
	{
		...
		{
			if(((Engine.engine.getGameAttribute("menu music is playing") : Bool) == false))
			{
				loopSoundOnChannel(getSound(178), 23);
				Engine.engine.setGameAttribute("menu music is playing", true);
			}
			else
			{
